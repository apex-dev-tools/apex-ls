...
import com.nawforce.pkgforce.types.ClassTypeDeclaration
...
class SubsetComparatorTest extends AnyFunSpec with DeclarationGeneratorHelper {
  ...
  private def generateEmptyClassDeclaration(name: String): ClassTypeDeclaration = {
    val ctd = new ClassTypeDeclaration(null, name, null)
    ctd._id = toId(name)
    ctd
  }
  ...
  describe("Annotations") {
    ...
    it("should fail even when Annotations are subsets") {
      ...
      //When
      val comparator =
        SubsetComparator(first.asInstanceOf[IMutableModuleTypeDeclaration], getMockResolver(), getMockResolver(Array("QualifiedName")))
  ...
  describe("Class ImplementsTypeList") {
    it("should be subsets when implementsTypeList type has a fully qualified name") {
      ...
      //When
      val comparator =
        SubsetComparator(first.asInstanceOf[IMutableModuleTypeDeclaration], getMockResolver(), getMockResolver(Array("fflib_BatchJob")))
      comparator.subsetOf(second)
  ...
  describe("Constructors") {
    ...
    it("should not be subsets when it typeRef has resolved names") {
      ...
      //When
      val comparator =
        SubsetComparator(first.asInstanceOf[IMutableModuleTypeDeclaration], getMockResolver(), getMockResolver(Array("ResolvedName")))
  ...
  describe("Methods") {
    ...
    it("should be subsets when parameterList have all resolved name") {
      ...
      val comparator =
        SubsetComparator(first.asInstanceOf[IMutableModuleTypeDeclaration], getMockResolver(), getMockResolver(Array("ResolvedName", "Bar")))
      comparator.subsetOf(second)
  ...
  describe("Fields") {
    ...
    it("should be subsets when type has resolved name") {
      ...
      //When
      val comparator =
        SubsetComparator(first.asInstanceOf[IMutableModuleTypeDeclaration], getMockResolver(), getMockResolver(Array("Foo")))
      comparator.subsetOf(second)
  ...
  describe("InnerClasses") {
    ...
    it("Should be subsets when inner types has typeRefs with names that are resolved") {
      ...
      //When
      val comparator =
        SubsetComparator(first.asInstanceOf[IMutableModuleTypeDeclaration], getMockResolver(), getMockResolver(Array("Bar")))
      comparator.subsetOf(second)
